...
operators \op{exit}, \op{stop}, or \op{abort} within the dynamic
context of the loop body. \op{exit} terminates the execution of the
current loop and resumes execution of the object following the loop
operator of the terminated loop. \op{exitto} is a generalized
\op{exit} -- it transfers control from the current point to the object
following the enclosing \op{exitlabel} operator. Matching means that
the /name matches.
