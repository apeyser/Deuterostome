...
and \op{rsendsig}. Children can be reaped with \op{waitpid}, which
takes a pid and return an exit value when the child exits (and
...
termination (shifted by 8 bits leftward, of course). The signal in
this case is the raw operating system values and not the internal,
architecture free signals. This value could be propagated directly by
calling \op{die} on it. \op{checkpid} is the non-blocking version; it
returns false if the child hasn't died, and true and the exit value if
it has died (however, it still leaves the child unreaped, requiring an
