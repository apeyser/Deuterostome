...
AC_DEFUN([CF_MSG_CBLAS], [dnl
 AC_MSG_CHECKING([for cblas])
 AC_MSG_RESULT([$1])dnl
])

#  CF_CBLAS([ACTION-IF-FOUND[, ACTION-IF-NOT-FOUND]])
AC_DEFUN([CF_CBLAS], [dnl
  AC_REQUIRE([ACX_BLAS])
  ...
  if test x"$acx_blas_ok" = xyes ; then
    AC_DEFINE([HAVE_BLAS],1,[Define if you have a BLAS library.])
	  ...
	  save_LIBS="$LIBS" ; LIBS="$BLAS_LIBS $LIBS"
	  AC_TRY_LINK_FUNC([cblas_sgemm], [
	    CF_MSG_CBLAS([yes, usable cblas available])
        cf_cblas_ok="yes"
      ], [
        CF_MSG_CBLAS([no, usable cblas not found])
      ])
	  LIBS="$save_LIBS"
	])
  else
    CF_MSG_CBLAS([no, no available cblas library])
  fi dnl

  if test x"$cf_cblas_ok" = xyes ; then
    AC_DEFINE([HAVE_CBLAS_H], [1], [Define if you have cblas.h.])
    ifelse([$1],,,[$1])
  else
    ifelse([$2],,:,[$2])
  fi
