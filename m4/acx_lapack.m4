...
AC_DEFUN([CF_MSG_CLAPACK], [dnl
 AC_MSG_CHECKING([for clapack])
 AC_MSG_RESULT([$1])dnl
])

AC_DEFUN([CF_CLAPACK], [dnl
  ...
  if test x"$acx_lapack_ok" = xyes ; then
	...
	AC_CHECK_HEADERS([clapack.h], [
	  save_LIBS="$LIBS" ; LIBS="$LAPACK_LIBS $BLAS_LIBS $LIBS $FLIBS"
	  AC_TRY_LINK_FUNC([dgbtrf_], [
	    CF_MSG_CLAPACK([yes, usable clapack.h found])
	  	cf_clapack_ok="yes"
      ], [
        CF_MSG_CLAPACK([no, usable clapack.h found])
	  ])
	  LIBS="$save_LIBS"
    ])
  else
    CF_MSG_CLAPACK([no, no available clapack library])
  fi

  if test x"$cf_clapack_ok" = xyes ; then
    AC_DEFINE([HAVE_CLAPACK_H], [1], [Define if you have clapack.h])
	ifelse([$1],,,[$1])
  else
    ifelse([$2],,:,[$2])
  fi dnl
