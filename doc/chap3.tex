...
active_obj         & inter_lock_implicit & ...                   \\
bool               & inter_lock_set      & --                    \\
--                 & getlibdir           & string                \\
...
man \dcomp{dnode} or \dcomp{dpawn}. \op{inter_lock_implicit} behaves
the same way for an arbitrary chunk of d-code. The operators
\op{inter_lock} and \op{inter_unlock} force an inter-process semaphore
lock or unlock respectively while executing its operand, regardless of
the \op{inter_lock_set} state.
...
machine). \emph{getlibdir} inquires the directory path where libraries
for the \dcomp{dnode} are stored (this path is established when the
\dcomp{dnode} is started). \emph{loadlib} loads a library of
operators; libraries are stored from the top of the VM downwards
(competing for VM space with D objects, which are stored from bottom
up) and returns the dictionary of the extrinsic operators contained in
that library. \emph{loadlib} also merges the extrinsic operators of
the library into \emph{systemdict} (with the exceptions of \emph{hi}
and \emph{libnum}). You cannot load multiple copies of the same
library. A library dictionary must include two obligatory operators
with library-specific effects, \emph{hi} and \emph{libnum}. \emph{hi}
